...
void display(Shader shader, Model carril, Model personaje, Model personaje2, Model futbolito, Model tasas, Model basketball, Model jugos, Model barrera, Model carpa_fut)//ahora si resiven parametros: shader, carril y personaje
{
	shader.use();
	...
	model = glm::scale(model, glm::vec3(0.05f, 0.060f, 0.05f));
	shader.setMat4("model", model);
	carril.Draw(shader);//para dibujar el modelo del personaje

	//------------------moantaña rusa_ si jala xD----------------
	//my_cilynder.roller_coaster();
	//----------------montaña rusa-------------------------------
	...
	//personaje1
	model = glm::translate(tmp, glm::vec3(-30.0f, 6.0f, 0.0));
	...
	//personaje 2
	model = glm::translate(tmp, glm::vec3(-30.0f, 6.0f, 10.0));
	model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
	//model = glm::rotate(model, glm::radians(rot_personaje), glm::vec3(0.0f, 1.0f, 0.0f));
	...
	//futbolitos
	model = glm::translate(tmp, glm::vec3(0.0f, 6.0f, -25.0));
	...
	futbolito.Draw(shader);
	//-----------------------------------futbolitos-------------------------------------
	model = glm::translate(tmp, glm::vec3(-5.0f, 6.0f, -28.0));
	...
	//model = glm::rotate(model, glm::radians(rot_llanta), glm::vec3(1.0f, 0.0f, 0.0f));
	shader.setMat4("model", model);
	tasas.Draw(shader);	//Izq delantera

	//basketball
	model = glm::translate(tmp, glm::vec3(0.0f, 5.5f, 30.0));
	model = glm::scale(model, glm::vec3(0.3f, 0.3f, 0.3f));
	model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));

	//model = glm::rotate(model, glm::radians(rot_llanta), glm::vec3(1.0f, 0.0f, 0.0f));
	shader.setMat4("model", model);
	basketball.Draw(shader);	//Izq delantera
	...
	//jugos
	model = glm::translate(tmp, glm::vec3(-25.0f, 5.8f, 20.0));
...
int main()
{
	...
	// Load models
	Model carril =((char *)"Models/parque/parque.obj");//para cargar el modelo del carril
	...
	Model tasas = ((char *)"Models/tasas_giratorias/tasas_giratorias.obj");
...
//ya que el pivote esta mal ubicado en el modelo de la llanta
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		display(modelShader, carril, personaje, personaje2, futbolito, tasas, basketball, jugos, barrera, carpa_fut);
