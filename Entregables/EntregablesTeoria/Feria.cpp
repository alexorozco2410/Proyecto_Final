...
/*--------------------Controles:
						...
						F: Activas el juego de los patos. 
						RIGHT_SHIFT + C: Te traslada a la camara donde esta el juego de patos
						RIGHT_SHIFT + B: Te devuelve a donde estabas antes de ir al juego de patos.
						Mouse: Dependiendo de la posicion del Mouse, la cámra gira hacia un lado u otro
						 */
#include <glew.h>
#include <glfw3.h>
#include <stb_image.h>
...
y_camera = -1.1f,
z_camera = 60.10f;
glm::vec3 camera2_position(-3.0f, -1.1f, 60.0f);
glm::vec3 camera2_position_aux(-3.0f, -1.1f, 60.0f);//para guardar la posicion de la camara al momento de hacer el cambio
Camera camera2(camera2_position);
double	lastX = 0.0f,
lastY = 0.0f;
bool firstMouse = true;
bool active_camera2 = true;

//camara patos
float x_camera_patos = -15.0f,
y_camera_patos = -1.1f,
z_camera_patos = -10.00f;
glm::vec3 camera_patos_position(-32.5f, -1.1f, -1.0f);//posicion inicial de la camara del juego de patos
glm::vec3 camera_patos_position_aux(-15.0f, -1.1f, -20.0f);////para guardar la posicion de la camara al momento de hacer el cambio
Camera camera_patos(camera_patos_position);
double	lastX_patos = 0.0f,
lastY_patos = 0.0f;
bool active_camera_patos = false;
...
//luz posicional
glm::vec3 lightPosPosition(-25.0f, 50.0f, 12.0f);
...
float duck_mov1 = 0.0f;
float duck_mov2 = 0.0f;
int vueltas = 0;
float duck_pos1 = -9.5f;
float duck_pos2 = -9.5f;
bool activate_ducks = false;
int direction_ducks1 = 0;
int direction_ducks2 = 1;
bool up_ducks1 = false;
bool up_ducks2 = true;
float rotpato_sup = 0.0f;
float rotpato_inf = 180.0f;

//arma
glm::vec3 gun_position(-3.0f, 5.4f, -38.0f);
glm::vec3 gun_position_aux1(0.0f, 0.0f, 0.0f);
glm::vec3 gun_position_aux2(-0.0f, 0.0f, -0.0f);
//-------------------------------------------------------
...
void animate_ducks(void) {
	if (activate_ducks) {
		if (direction_ducks1 == 0) {
			duck_pos1 -= 0.2f;
		}
		if (direction_ducks1 == 1) {
			duck_pos1 += 0.2f;
		}
		...
		if (direction_ducks2 == 1) {
			duck_pos2 += 0.2f;
		}
		if (direction_ducks2 == 0) {
			duck_pos2 -= 0.2f;
		}
...
void display(Shader shader, Shader lampshader, Shader lightingshader, Shader textureshader, Shader lightposshader, Shader shaderlight,
	...
	Model toilet, Model base_caballos, Model giro_caballos, Model caballo, Model coaster_cart, Model local, Model duck, Model tubo, Model arma)//ahora si resiven parametros: shader, carril y personaje
{
	//lightingshader.use();
	...
	glm::mat4 tmp_ducks = glm::mat4(1.0f);
	glm::mat4 tmp_game = glm::mat4(1.0f);
	...
	model = glm::translate(tmp, glm::vec3(31.5f, 5.68f, -20.0));
	model = glm::scale(model, glm::vec3(0.015f, 0.015f, 0.015f));
	model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	...
	*/
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	//------------------------------- juego de patos ------------------------------------------
	model = glm::translate(tmp, glm::vec3(-6.0f, 5.5f, -40.0));
	...
	model = glm::translate(model, glm::vec3(00.0f, -0.5f, duck_pos1));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov1, 0.0f));
	shader.setMat4("model", model);
	tmp_ducks = model;
	tubo.Draw(shader);

	model = glm::translate(model, glm::vec3(-1.0f, 4.8f, 3.7f));
	model = glm::rotate(model, glm::radians(rotpato_inf), glm::vec3(0.0f, 1.0f, 0.0f));
	shader.setMat4("model", model);
	duck.Draw(shader);
	...
	model = tmp_game;
	model = glm::translate(model, glm::vec3(00.0f, -0.5f, duck_pos1));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov2, 3.0f));
	shader.setMat4("model", model);
	tubo.Draw(shader);
	...
	model = tmp_game;
	model = glm::translate(model, glm::vec3(00.0f, -0.5f, duck_pos1));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov1, 6.0f));
	shader.setMat4("model", model);
	tubo.Draw(shader);
	...
	model = tmp_game;
	model = glm::translate(model, glm::vec3(00.0f, -0.5f, duck_pos1));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov2, 9.0f));
	shader.setMat4("model", model);
	tubo.Draw(shader);
	...
	model = tmp_game;
	model = glm::translate(model, glm::vec3(00.0f, -0.5f, duck_pos1));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov1, 12.0f));
	shader.setMat4("model", model);
	tubo.Draw(shader);
	...
	model = glm::translate(model, glm::vec3(-2.0f, 2.5f, duck_pos2));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov2, 0.0f));
	model = glm::scale(model, glm::vec3(0.45f, 1.0f, 1.0f));
	shader.setMat4("model", model);
	tmp_ducks = model;
	tubo.Draw(shader);
	...
	model = glm::rotate(model, glm::radians(rotpato_sup), glm::vec3(0.0f, 1.0f, 0.0f));
	shader.setMat4("model", model);
	duck.Draw(shader);
	...
	model = tmp_game;
	model = glm::translate(model, glm::vec3(-2.0f, 2.5f, duck_pos2));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov1, 3.0f));
	model = glm::scale(model, glm::vec3(0.45f, 1.0f, 1.0f));
	shader.setMat4("model", model);
	tubo.Draw(shader);
	...
	model = tmp_game;
	model = glm::translate(model, glm::vec3(-2.0f, 2.5f, duck_pos2));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov2, 6.0f));
	model = glm::scale(model, glm::vec3(0.45f, 1.0f, 1.0f));
	shader.setMat4("model", model);
	tubo.Draw(shader);
	...
	model = tmp_game;
	model = glm::translate(model, glm::vec3(-2.0f, 2.5f, duck_pos2));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov1, 9.0f));
	model = glm::scale(model, glm::vec3(0.45f, 1.0f, 1.0f));
	shader.setMat4("model", model);
	tubo.Draw(shader);
	...
	model = tmp_game;
	model = glm::translate(model, glm::vec3(-2.0f, 2.5f, duck_pos2));
	model = glm::translate(model, glm::vec3(00.0f, duck_mov2, 12.0f));
	model = glm::scale(model, glm::vec3(0.45f, 1.0f, 1.0f));
	shader.setMat4("model", model);
	tubo.Draw(shader);
	...
	//arma
	model = tmp_game;
	model = glm::translate(model, glm::vec3(3.5f, 3.47f, 0.0f));
	...
	model = glm::translate(tmp, gun_position);
	model = glm::translate(model, gun_position_aux1);//pone en el origen al arma
	model = glm::translate(model, gun_position_aux2);//pone junto a la camara el arma
	model = glm::scale(model, glm::vec3(0.06f, 0.06f, 0.06f));
	...
	model = glm::translate(tmp, glm::vec3(-7.0f, 5.68f, 32.0)); //entre carrusel y carros chocones
	model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
	...
	model = glm::translate(tmp, glm::vec3(21.0f, 5.68f, -33.0));//a la do de las tasas
	model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
	model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	shader.setMat4("model", model);
	poste_luz1.Draw(shader);

	model = glm::translate(tmp, glm::vec3(-15.0f, 5.68f, -33.0));//entre los patos y la rueda
	model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
	//model = glm::rotate(model, glm::radians(00.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	shader.setMat4("model", model);
	poste_luz1.Draw(shader);
	...
	shaderlight.use();
	glm::vec3 lightPos5(-31.8f, 3.5f, -36.0f);
	shaderlight.setVec3("lightColor", 1.0f, 1.0f, 1.0f);
	shaderlight.setVec3("lightPos", lightPos5);
	...
	//glDrawArrays(GL_TRIANGLES, 36, 73);	//Light
	...
	model = glm::translate(model, glm::vec3(-48.0f, 3.5f, 12.0f));
...
int main()
{
	...
	Shader shaderlight("shaders/shader_light.vs", "shaders/shader_light.fs");
	...
	Model local = ((char *)"Models/ducks_game/local.obj");
	Model duck = ((char *)"Models/ducks_game/duck.obj");
	Model tubo = ((char *)"Models/ducks_game/tubo.obj");
	Model arma = ((char *)"Models/arma/arma.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		display(modelShader, lampShader, lightingShader, projectionShader, lightposShader, shaderlight, carril, personaje, personaje2, futbolito, carpa_tasas, base_tasas, tasa, basketball, tiro_blanco, jugos, ambulante1, barrera, carpa_fut, bumper_car,
			pista, silla, trash, trash2, poste_luz1, arbol1, premios, toilet, base_caballos, giro_caballos, caballo, coaster_cart, local, duck, tubo, arma);
...
void my_input(GLFWwindow *window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS)//activar taas
		activate_tasas = true;
	if (glfwGetKey(window, GLFW_KEY_Y) == GLFW_PRESS)//descativar tasas
		activate_tasas = false;
	...
	if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS)//para activar el carro de comida
		activate_ducks = true;
	if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS) {
		if (glfwGetKey(window, GLFW_KEY_RIGHT_SHIFT)) {
			if (active_camera2) {
				camera2_position_aux = camera.Position;
				camera = camera_patos;
				//camera.Position =
				active_camera2 = false;
				active_camera_patos = true;
				gun_position = camera.Position;
				gun_position_aux1 = glm::vec3(30.0f, 1.1f, 0.0f);
				gun_position_aux2 = glm::vec3(-7.5f, 7.5f, -31.5f);
			}
		} 
	}
	//vuelve a la posicion donde te quedaste antes de trasladarte al juego
	if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS) {
		if (glfwGetKey(window, GLFW_KEY_RIGHT_SHIFT)) {
			//camera_patos_position_aux = camera.Position;
			camera = camera2;
			camera.Position = camera2_position_aux;
			active_camera2 = true;
			active_camera_patos = false;
			gun_position = glm::vec3(-3.0f, 5.4f, -38.0f);
			gun_position_aux1 = glm::vec3(0.0f, 0.0f, 0.0f);
			gun_position_aux2 = glm::vec3(0.0f, 0.0f, 0.0f);
		}
	}
}
